...
class Logger {
  ...
  constructor(namespace, options) {
    ...
    // this.firstElem = this._options.onlyFirstElem
    this.firstElem = null;
  ...
  log(args, options) {
    ...
    let logReturn = this.#handleOnlyFirstElem(logStack.stack);
    if (logReturn) {
      return logReturn;
    }

    this.#writeLog(logStack.stack)
   ...
   */
  #writeLog(stack){
    const logTitle = this.#formatLogCall(this.#callStackParser(stack));
  ...
  #callStackParser(callStack) {
    ...
    for (; logLineIndex < callStackPartsLen; logLineIndex++) {
      ...
      if (
        ...
        console.log(JSON.stringify(currentLineParts[1] + delimiter));
    ...
    //test - trimmedDelimiter
    //dev/prod - delimiter
    const checkDelimiter =
      process.env.NODE_ENV === "test" ? trimmedDelimiter : delimiter;
